...
| `app/admin.py` | admin corpus + review-progress views |
| `app/adminauth.py` | ADMIN_PASSWORD -> HMAC bearer token; `require_admin` dependency |
| `app/adminstore.py` | exercise CRUD as a Postgres overlay (`exercise_overrides`) on the committed JSON |
...
`POST /admin/login` · `GET /admin/stats` · `GET /admin/exercises[/{id}]` ·
`POST/PUT/DELETE /admin/exercises` · `POST /admin/exercises/{id}/review` ·
