    ...
    <div className="min-h-screen bg-navy-midnight text-white flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-navy-border">
          ...
          <div className="flex items-center gap-6 bg-navy-surface p-5 rounded-2xl border border-navy-border shadow-xl">
            <div>
              <span className="text-2xs uppercase tracking-wider text-slate font-semibold block">
                Coding Skill Index
              </span>
              <div className="text-4xl sm:text-5xl font-extrabold font-mono text-gradient-aqua">
                72%
              </div>
            </div>
            <div className="h-10 w-px bg-navy-border" />
            <div className="space-y-1 text-xs text-slate">
              <p><span className="font-bold text-white">18</span> challenges solved</p>
        ...
        <motion.div
          ...
          className="p-6 rounded-2xl border border-warning/30 bg-warning-subtle/20 flex flex-col md:flex-row items-start md:items-center justify-between gap-6"
        >
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Badge variant="warning" size="sm" dot>CURRENT WEAKNESS</Badge>
              <span className="text-xs text-slate font-mono">Catch rate: 43%</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-white">Coding Skill by Defect Taxonomy</h3>
              <span className="text-xs text-slate">Click category to practice</span>
            </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <Card dark className="p-5 border-aqua/30 bg-navy-surface relative overflow-hidden">
              <span className="text-2xs uppercase font-mono tracking-wider text-aqua font-semibold block mb-2">
                RECOMMENDED PRACTICE
              </span>
              <h4 className="text-base font-bold text-white mb-1">{recommendedEx.title}</h4>
              <p className="text-xs text-slate line-clamp-2 mb-4 leading-relaxed">
            ...
            <Card dark className="p-5">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-sm font-bold text-white">Recent Submissions</h4>
                <span className="text-2xs text-slate">Last 5 attempts</span>
              </div>
              ...
              <div className="space-y-3">
                {recentSubmissions.map((sub, i) => (
                  <div
                    ...
                    className="p-2.5 rounded-lg bg-navy-midnight border border-navy-border flex items-center justify-between hover:border-aqua/40 cursor-pointer transition-colors"
                  >
                    <div>
                      <p className="text-xs font-semibold text-white">{sub.title}</p>
                      <span className="text-2xs text-slate">{sub.class} · {sub.time}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-bold font-mono text-white block">{sub.score}%</span>
                      <span className={clsx(
